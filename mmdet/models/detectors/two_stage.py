...
class TwoStageDetector(BaseDetector):
    ...
    #filipe implementation - VERSÃO CORRIGIDA
    def my_get_features_logits(self, batch_inputs: Tensor,
        ...
        # ============================================================
        # CORREÇÃO: Extrair features corretamente para cada imagem
        # ============================================================
        
        # Para cada imagem, extrair features das suas próprias bboxes
        for img_idx, result in enumerate(results_list):
            bboxes = result['bboxes']
            num_boxes = bboxes.shape[0]
            
            if num_boxes == 0:
                # Sem boxes, criar tensor vazio
                result['feat'] = torch.empty((0, 1024), device=bboxes.device)
                continue
            
            # Criar ROIs para esta imagem específica
            indices = torch.full((num_boxes, 1), img_idx, device=bboxes.device)
            rois = torch.cat([indices, bboxes], dim=1)
            
            # Extrair features ROI
            roi_feats = self.roi_head.bbox_roi_extractor(x, rois)
            
            # Passar pelas FC layers
