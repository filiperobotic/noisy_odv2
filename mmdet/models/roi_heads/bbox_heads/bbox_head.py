...
class BBoxHead(BaseModule):
    ...
    #FILIPE CODE
    def _predict_by_feat_single_logit(
            self,
            roi: Tensor,
            cls_score: Tensor,
            bbox_pred: Tensor,
        ...
        Args:
            ...
            rcnn_test_cfg (obj:`ConfigDict`): `test_cfg` of Bbox Head.
                Defaults to None
            all_logits: If True, return all the logits of the bbox head, otherwise return only the logits of the predicted classes.
        ...
        if rcnn_test_cfg is None:
            ...
            # my_thr =scores.min().item()
            #det_bboxes, det_labels = multiclass_nms(
            det_bboxes, det_labels, logits = multiclass_nms_with_logits(
                bboxes,
                scores,
                logits, #FILIPE
                rcnn_test_cfg.score_thr,
