...
#FILIPE CODE
def multiclass_nms_with_logits(
    multi_bboxes: Tensor,
    multi_scores: Tensor,
    logits: Tensor,
    score_thr: float,
    ...
    if not all_logits :
        ...
        logits = logits.repeat_interleave(num_classes, dim=0)  # [1000 → 20000, 20]
    labels = labels.reshape(-1)
    ...
    if not torch.onnx.is_in_onnx_export():
        # NonZero not supported  in TensorRT
        inds = valid_mask.nonzero(as_tuple=False).squeeze(1)
        bboxes, scores, logits, labels = bboxes[inds], scores[inds], logits[inds], labels[inds]
    else:
        # TensorRT NMS plugin has invalid output filled with -1
        # add dummy data to make detection output correct.
        bboxes = torch.cat([bboxes, bboxes.new_zeros(1, box_dim)], dim=0)
    ...
    if return_inds:
        return dets, labels[keep], inds[keep], logits[keep]
    else:
