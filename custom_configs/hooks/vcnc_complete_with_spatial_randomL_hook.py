"""
Estratégia K: Random Relabeling

Ideia: Ao invés de filtrar amostras ruidosas, atribuir rótulos aleatórios.
Isso transforma ruído assimétrico (concentrado) em ruído simétrico (espalhado).

Baseado no conceito de "Machine Unlearning via Random Labeling":
- Modelo "desaprende" associações erradas quando vê a mesma amostra com labels diferentes
- Ruído simétrico é mais fácil de lidar do que assimétrico

Lógica:
- Detecta suspeitos com margem pequena (possível ruído assimétrico)
- Ao invés de filtrar, atribui rótulo aleatório (excluindo GT e pred)
- Mantém relabeling normal para margem grande (possível ruído simétrico)
"""
...
class VCNCRandomRelabelingHook(Hook):
    """
    VCNC Estratégia K: Random Relabeling
    
    Ao invés de filtrar amostras com margem pequena (possível ruído assimétrico),
    atribui um rótulo aleatório para transformar em ruído simétrico.
    
    Lógica:
    - Margem grande → RELABELA para classe dominante (simétrico normal)
    - Margem pequena → RANDOM LABEL (transforma assimétrico em simétrico)
    """
    ...
    def __init__(self,
                 ...
                 # Random labeling
                 exclude_gt_and_pred: bool = True,  # Excluir GT e pred do random
                 exclude_dominant: bool = True,  # Excluir classe dominante do cluster
    ...
    def _get_random_label(self, gt_label, pred_label, dominant_label):
        """
        Escolhe um rótulo aleatório, excluindo certas classes.
        """
        excluded = set()
        ...
        if len(available) == 0:
            # Fallback: retorna qualquer classe diferente da GT
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[RandomRelabel] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[RandomRelabel] ========== Época {epoch} ==========")
        ...
        if self.debug:
            print(f"[RandomRelabel] Total boxes: {len(all_box_data)}")
            margins = [b['margin'] for b in all_box_data]
            print(f"[RandomRelabel] Margem média: {np.mean(margins):.3f}, std: {np.std(margins):.3f}")
        ...
        if self.debug:
            print(f"[RandomRelabel] Fase: {criteria['phase']}")
        ...
        stats = {
            'large_margin_relabel': 0,
            'small_margin_random': 0,
        }
        ...
        for cluster_id, cluster_boxes in clusters.items():
            ...
            for box in cluster_boxes:
                ...
                if similarity > c_similarity:
                    # ============================================================
                    # DECISÃO BASEADA EM MARGEM: RELABEL vs RANDOM
                    # ============================================================
                    if box['margin'] > self.margin_threshold:
                        # Margem grande → modelo confiante → RELABELA para dominante
                        self._apply_relabel(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'], dominant_label)
                        box['action'] = 'relabel'
                        stats['large_margin_relabel'] += 1
                    else:
                        # Margem pequena → possível assimétrico → RANDOM LABEL
                        ...
                        box['action'] = 'random'
                        stats['small_margin_random'] += 1
        
        if self.debug:
            print(f"[RandomRelabel] Total âncoras: {total_anchors} ({total_anchors/len(all_box_data)*100:.2f}%)")
            print(f"[RandomRelabel] Total suspeitos: {total_suspects}")
            print(f"[RandomRelabel] Decisões (margin_threshold={self.margin_threshold}):")
            print(f"  - Margem grande → Relabel dominante: {stats['large_margin_relabel']}")
            print(f"  - Margem pequena → Random label: {stats['small_margin_random']}")
        ...
        if self.enable_spatial_refinement:
            for img_path, img_boxes in boxes_by_image.items():
                if len(img_boxes) < 2:
                    continue
                active_boxes = [b for b in img_boxes if b['action'] != 'random']  # Não aplica spatial em random
            ...
            if self.debug:
                print(f"[RandomRelabel] Relabelados por spatial: {spatial_relabel_count}")
        
        if self.debug:
            print(f"[RandomRelabel] ==========================================\n")
