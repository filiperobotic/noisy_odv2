"""
Estratégia A: Filtragem Baseada em Clusters e Âncoras + Spatial Refinement

Ideia: Usa a mesma estrutura do VCNC (clusters, âncoras, similaridade),
mas em vez de RELABELA, apenas FILTRA boxes que seriam relabelados.

Lógica:
- Identifica âncoras (boxes confiáveis) em cada cluster
- Para boxes suspeitos que estão em clusters com âncoras de OUTRA classe:
  - Em vez de relabela → FILTRA (remove do treinamento)
- Isso evita amplificar erros no ruído assimétrico
- Spatial Refinement MANTIDO: corrige contaminação espacial (problema diferente)
"""
...
class VCNCFilterOnlyWithClustersHook(Hook):
    """
    VCNC com Clusters e Âncoras, mas apenas Filtragem (sem Relabeling)
    + Spatial Refinement (mantido pois corrige problema diferente)
    """
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[FilterClusters] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[FilterClusters] ========== Época {epoch} ==========")
        ...
        if self.debug:
            print(f"[FilterClusters] Total boxes: {len(all_box_data)}")
        ...
        if self.debug:
            print(f"[FilterClusters] Fase: {criteria['phase']}, Clusters: {len(set(cluster_ids))}")
        ...
        for cluster_id, cluster_boxes in clusters.items():
            ...
            for box in cluster_boxes:
                ...
                # ============================================================
                # DIFERENÇA DO VCNC ORIGINAL: em vez de RELABELA → FILTRA
                # ============================================================
        ...
        if self.debug:
            print(f"[FilterClusters] Total âncoras: {total_anchors} ({total_anchors/len(all_box_data)*100:.2f}%)")
            print(f"[FilterClusters] Total suspeitos: {total_suspects} ({total_suspects/len(all_box_data)*100:.2f}%)")
            print(f"[FilterClusters] Filtrados por clustering: {filter_count} ({filter_count/len(all_box_data)*100:.2f}%)")
        
        # ============================================================
        # SPATIAL REFINEMENT (mantido - corrige problema diferente)
        # ============================================================
        spatial_relabel_count = 0
        
        if self.enable_spatial_refinement:
            if self.debug:
                print(f"[FilterClusters] Aplicando Spatial Refinement...")
            ...
            if self.debug:
                print(f"[FilterClusters] Relabelados por spatial: {spatial_relabel_count}")
        
        if self.debug:
            print(f"[FilterClusters] ==========================================\n")
