"""
Visual Clustering Noise Correction Hook - VERSÃO COMPLETA COM SPATIAL REFINEMENT E DIAGNÓSTICO
...
5. DIAGNÓSTICO: Coleta estatísticas sobre âncoras erradas, suspeitos não tratados, etc.
...
class VCNCCompleteWithSpatialHook(Hook):
    ...
    def before_train_epoch(self, runner):
        ...
        for batch_idx, data_batch in enumerate(dataloader):
            ...
            for i, data_sample in enumerate(data_batch['data_samples']):
                ...
                for gt_idx in range(assign_result.num_gts):
                    ...
                    # Calcular margem (diferença entre top1 e top2)
        ...
        if self.enable_clustering_relabel:
            ...
            # === DIAGNÓSTICO: Variáveis para coleta ===
            all_anchors = []
            ...
            for cluster_id, cluster_boxes in clusters.items():
                ...
                if len(anchors) == 0:
                    # DIAGNÓSTICO: Boxes ruidosos neste cluster não serão tratados
                    for box in cluster_boxes:
                ...
                if consensus_ratio < c_consensus:
                    # DIAGNÓSTICO: Sem consenso - boxes suspeitos não serão tratados
                    for box in cluster_boxes:
                ...
                for box in cluster_boxes:
                    ...
                    if similarity > c_similarity:
                        ...
                        # DIAGNÓSTICO: Suspeito não similar às âncoras
                        all_suspects_not_similar.append(box)
            
            # === DIAGNÓSTICO: Coletar e salvar estatísticas ===
            if self.enable_diagnostic:
                diag_stats = self._collect_diagnostic_stats(
                    epoch, all_box_data, all_anchors, 
                    all_suspects_treated, all_suspects_not_similar, 
                    all_noisy_not_treated
                )
    ...
    def _collect_diagnostic_stats(self, epoch, all_box_data, anchors, 
                                   suspects_treated, suspects_not_similar, 
                                   noisy_not_treated):
        """Coleta e salva estatísticas de diagnóstico."""
        
        stats = {
            'epoch': epoch,
            'total_boxes': len(all_box_data),
            'total_anchors': len(anchors),
            'suspects_treated': len(suspects_treated),
            'suspects_not_similar': len(suspects_not_similar),
            'noisy_not_treated': len(noisy_not_treated),
        }
        
        # === ÂNCORAS POTENCIALMENTE ERRADAS ===
        anchor_gt_ne_pred = [a for a in anchors if a['gt_label'] != a['pred_label']]
        stats['anchors_gt_ne_pred'] = len(anchor_gt_ne_pred)
        stats['anchors_gt_ne_pred_pct'] = len(anchor_gt_ne_pred) / len(anchors) * 100 if len(anchors) > 0 else 0
        
        # Âncoras com margem baixa
        anchor_low_margin = [a for a in anchors if a.get('margin', 1) < 0.3]
        stats['anchors_low_margin'] = len(anchor_low_margin)
        stats['anchors_low_margin_pct'] = len(anchor_low_margin) / len(anchors) * 100 if len(anchors) > 0 else 0
        
        # Pares errados em âncoras
        wrong_anchor_pairs = defaultdict(int)
        for a in anchor_gt_ne_pred:
            pair = (a['gt_label'], a['pred_label'])
            wrong_anchor_pairs[pair] += 1
        stats['wrong_anchor_pairs'] = [
            {'gt': int(gt), 'pred': int(pred), 'count': count}
            for (gt, pred), count in sorted(wrong_anchor_pairs.items(), key=lambda x: -x[1])[:10]
        ]
        
        # === MARGENS ===
        all_margins = [b.get('margin', 0) for b in all_box_data]
        if all_margins:
            stats['margin_mean'] = float(np.mean(all_margins))
            stats['margin_std'] = float(np.std(all_margins))
            stats['margin_median'] = float(np.median(all_margins))
        
        # Margens dos suspeitos tratados
        if len(suspects_treated) > 0:
            suspect_margins = [s.get('margin', 0) for s in suspects_treated]
            stats['suspect_margin_mean'] = float(np.mean(suspect_margins))
            stats['suspect_margin_median'] = float(np.median(suspect_margins))
        
        # === SUSPEITOS NÃO TRATADOS ===
        if len(suspects_not_similar) > 0:
            sims = [s.get('similarity_to_anchors', 0) for s in suspects_not_similar]
            stats['not_similar_sim_mean'] = float(np.mean(sims))
            stats['not_similar_sim_max'] = float(np.max(sims))
            
            not_sim_classes = Counter([s['gt_label'] for s in suspects_not_similar])
            stats['not_similar_top_classes'] = [
                {'class': int(cls), 'count': count}
                for cls, count in not_sim_classes.most_common(5)
            ]
        
        # === PARES CONFUSOS ===
        all_suspects = suspects_treated + suspects_not_similar
        if len(all_suspects) > 0:
            confusion_pairs = defaultdict(int)
            for s in all_suspects:
                pair = tuple(sorted([s['gt_label'], s['pred_label']]))
        ...
        # === PRINT RESUMO ===
        if self.debug:
            print(f"\n[VCNC-DIAG] ========== DIAGNÓSTICO Época {epoch} ==========")
            print(f"[VCNC-DIAG] Total boxes: {stats['total_boxes']}")
            print(f"[VCNC-DIAG] Âncoras: {stats['total_anchors']} ({stats['total_anchors']/stats['total_boxes']*100:.1f}%)")
            print(f"[VCNC-DIAG] Suspeitos tratados: {stats['suspects_treated']}")
            print(f"[VCNC-DIAG] Suspeitos NÃO similares: {stats['suspects_not_similar']}")
            print(f"[VCNC-DIAG] Ruidosos sem âncora/consenso: {stats['noisy_not_treated']}")
            print(f"[VCNC-DIAG]")
            print(f"[VCNC-DIAG] === ÂNCORAS POTENCIALMENTE ERRADAS ===")
            print(f"[VCNC-DIAG] Âncoras com GT != Pred: {stats['anchors_gt_ne_pred']} ({stats['anchors_gt_ne_pred_pct']:.1f}%)")
            print(f"[VCNC-DIAG] Âncoras com margem < 0.3: {stats['anchors_low_margin']} ({stats['anchors_low_margin_pct']:.1f}%)")
            if stats.get('wrong_anchor_pairs'):
                print(f"[VCNC-DIAG] Top pares errados: {stats['wrong_anchor_pairs'][:3]}")
            if stats.get('confusion_pairs'):
                print(f"[VCNC-DIAG] Top pares confusos (suspeitos): {stats['confusion_pairs'][:3]}")
            print(f"[VCNC-DIAG] ================================================\n")
    ...
    def after_train(self, runner):
        """Salva todas as estatísticas ao final do treinamento."""
        if self.enable_diagnostic and self.all_diagnostic_stats:
            ...
            if self.debug:
                print(f"[VCNC-DIAG] Estatísticas salvas em {output_file}")
