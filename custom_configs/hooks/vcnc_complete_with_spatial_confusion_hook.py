"""
Estratégia G: Matriz de Confusão Dinâmica

Ideia:
- Construir matriz de confusão a cada época
- Identificar pares (A,B) onde confusão é alta (A→B e B→A)
- Para esses pares: apenas FILTRA, nunca relabela
- Para outros pares: RELABELA normalmente

Esta é a única estratégia que tenta detectar automaticamente 
quais pares de classes são "perigosos" para relabeling.
"""
...
class VCNCConfusionMatrixHook(Hook):
    """
    VCNC Estratégia G: Matriz de Confusão Dinâmica
    
    Lógica:
    - Detecta pares de classes confusas automaticamente
    - Se gt_label e dominant_label formam par confuso → FILTRA
    - Caso contrário → RELABELA
    """
    ...
    def __init__(self,
                 ...
                 # Parâmetros da detecção de pares confusos
                 confusion_threshold: float = 0.15,  # Se confusão > 15% → par confuso
                 min_samples_for_confusion: int = 50,  # Mínimo de amostras para calcular
        ...
        self.min_samples_for_confusion = min_samples_for_confusion
        ...
        # Pares confusos detectados (persistem entre épocas)
    ...
    def _detect_confused_pairs(self, all_box_data):
        """
        Detecta pares de classes que o modelo confunde frequentemente.
        Confusão bidirecional: se A→B e B→A são frequentes, é par confuso.
        """
        # Contar confusões
        ...
        for box in all_box_data:
            ...
            pred_label = box['pred_label']
            class_totals[gt_label] += 1
        ...
        for gt_class, pred_counts in confusion_counts.items():
            total = class_totals[gt_class]
            if total < self.min_samples_for_confusion:
                continue
            
            for pred_class, count in pred_counts.items():
                confusion_rate = count / total
                
                if confusion_rate > self.confusion_threshold:
                    # Par ordenado para evitar duplicatas
                    pair = tuple(sorted([gt_class, pred_class]))
                    confused_pairs.add(pair)
        
        return confused_pairs
    
    def _is_confused_pair(self, class_a, class_b):
        """Verifica se duas classes formam um par confuso."""
        pair = tuple(sorted([class_a, class_b]))
        return pair in self.confused_pairs
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[ConfusionMatrix] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[ConfusionMatrix] ========== Época {epoch} ==========")
        ...
        if self.debug:
            print(f"[ConfusionMatrix] Total boxes: {len(all_box_data)}")
        
        # Detectar pares confusos (atualiza a cada época)
        self.confused_pairs = self._detect_confused_pairs(all_box_data)
        
        if self.debug:
            print(f"[ConfusionMatrix] Pares confusos detectados: {len(self.confused_pairs)}")
            if len(self.confused_pairs) <= 10:
                for pair in self.confused_pairs:
                    print(f"  - Classe {pair[0]} ↔ Classe {pair[1]}")
        ...
        if self.debug:
            print(f"[ConfusionMatrix] Fase: {criteria['phase']}")
        ...
        for cluster_id, cluster_boxes in clusters.items():
            ...
            for box in cluster_boxes:
                ...
                if similarity > c_similarity:
                    # ============================================================
                    # DECISÃO BASEADA EM PAR CONFUSO
                    # ============================================================
                    if self._is_confused_pair(box['gt_label'], dominant_label):
                        # Par confuso → FILTRA (não arrisca relabeling)
                        self._apply_ignore_flag(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'])
                        box['filtered'] = True
                        filter_count += 1
                        stats['confused_pair_filter'] += 1
                    else:
                        # Par seguro → RELABELA
                        self._apply_relabel(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'], dominant_label)
                        box['relabeled'] = True
                        relabel_count += 1
                        stats['safe_pair_relabel'] += 1
        
        if self.debug:
            print(f"[ConfusionMatrix] Total âncoras: {total_anchors} ({total_anchors/len(all_box_data)*100:.2f}%)")
            print(f"[ConfusionMatrix] Total suspeitos: {total_suspects}")
            print(f"[ConfusionMatrix] Decisões:")
            print(f"  - Par confuso → Filtra: {stats['confused_pair_filter']}")
            print(f"  - Par seguro → Relabel: {stats['safe_pair_relabel']}")
        ...
        if self.enable_spatial_refinement:
            ...
            if self.debug:
                print(f"[ConfusionMatrix] Relabelados por spatial: {spatial_relabel_count}")
