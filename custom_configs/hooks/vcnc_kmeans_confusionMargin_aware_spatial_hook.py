...
class VCNCKMeansConfusionMarginHook(Hook):
    """
    ...
    def __init__(self,
                 ...
                 # === GATE DE CONFUSÃO + AÇÃO MARGIN-BASED ===
                 # Quando o gate detecta par ruidoso, em vez de filtrar/pular o
                 # relabel, aplica a lógica margin-based: se a margem entre
                 # top-1 e top-2 do modelo é alta o suficiente, relabela para
                 # top-1; se baixa, filtra.
    ...
    def _fit_gmm_per_class(self, scores_by_class):
        ...
        for cls_id, scores in scores_by_class.items():
            ...
            try:
                ...
                if self.debug:
                    print(f"[VCNC-KM-CM] Erro GMM classe {cls_id}: {e}")
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[VCNC-KM-CM] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[VCNC-KM-CM] ========== Época {epoch} ==========")
        ...
        #     print("[VCNC-KM-CM] ERRO: Esperado ConcatDataset")
        #     return
        ...
        # ============================================================
        if self.debug:
            print("[VCNC-KM-CM] Coletando dados...")
        ...
        if len(all_box_data) == 0:
            print("[VCNC-KM-CM] Nenhum box coletado!")
            return
        
        if self.debug:
            print(f"[VCNC-KM-CM] Coletados {len(all_box_data)} boxes em {len(boxes_by_image)} imagens")
        ...
        if gate_active:
            ...
            if self.debug:
                print(f"\n[VCNC-KM-CM] === GATE DE CONFUSÃO ===")
                print(f"[VCNC-KM-CM] Pares válidos analisados: {confusion_stats['n_valid_pairs']}")
                print(f"[VCNC-KM-CM] Pares marcados como ruidosos: {confusion_stats['n_confused_pairs']}")
                print(f"[VCNC-KM-CM] Severidade — mediana: {confusion_stats['median']:.4f}, "
                      f"MAD: {confusion_stats['mad']:.4f}")
                print(f"[VCNC-KM-CM] Threshold = max(med+{self.confusion_gate_mad_factor}*MAD, "
                      f"{self.confusion_gate_ratio_factor}*med) = "
                      f"{confusion_stats['threshold']:.4f}")
                print(f"[VCNC-KM-CM] Top 10 pares por severidade:")
                for tp in confusion_stats['top_pairs']:
                    flag = "  ★ RUIDOSO" if tp['is_confused'] else ""
                    i, j = tp['pair']
                    print(f"[VCNC-KM-CM]   classes ({i}, {j}): sev={tp['severity']:.4f} "
                          f"C[{i}->{j}]={tp['C_ij']:.4f}  C[{j}->{i}]={tp['C_ji']:.4f}{flag}")
        else:
            if self.debug:
                print(f"[VCNC-KM-CM] Gate de confusão DESATIVADO nesta época.")
        
        gate_skip_count = {'confidence': 0, 'clustering': 0}
        gate_filter_count = {'confidence': 0, 'clustering': 0}
        margin_count = {'relabeled': 0, 'filtered': 0, 'kept': 0}
        ...
        if self.enable_confidence_relabel:
            if self.debug:
                print(f"\n[VCNC-KM-CM] ETAPA 1: Relabel por confiança > {self.relabel_confidence_threshold}")
            
            for box in all_box_data:
                if (box['pred_score'] > self.relabel_confidence_threshold and 
                    box['pred_label'] != box['gt_label']):
                    
                    # GATE: par ruidoso detectado em Etapa 1 → filtra
                    # (Em Etapa 1 já passou a checagem pred_score > 0.9, então
                    # margin-based equivaleria a "relabel sempre"; preferimos
                    # a ação conservadora de filtrar.)
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Relabelados por confiança: {confidence_relabel_count} "
                      f"({confidence_relabel_count/len(all_box_data)*100:.2f}%)")
                if gate_active:
                    print(f"[VCNC-KM-CM] Gate Etapa 1 — skip: {gate_skip_count['confidence']}, "
        ...
        if self.enable_clustering_relabel:
            if self.debug:
                print(f"\n[VCNC-KM-CM] ETAPA 2: Relabel por clustering visual")
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Fase: {criteria['phase']}, Clusters: {len(set(cluster_ids))}")
            ...
            for cluster_id, cluster_boxes in clusters.items():
                ...
                for box in cluster_boxes:
                    ...
                    if similarity > c_similarity:
                        # GATE: par ruidoso detectado → aplica margin-based
                        if gate_active and (box['gt_label'], dominant_label) in confused_pairs:
                            top2 = box['scores'].topk(2)
                            margin = (top2.values[0] - top2.values[1]).item()
                            top1_label = top2.indices[0].item()

                            if margin < self.margin_threshold:
                                # Modelo confuso: filtra
                                self._apply_ignore_flag(
                                    datasets,
                                    box['sub_idx'],
                                    box['data_idx'],
                                ...
                                box['filtered'] = True
                                margin_count['filtered'] += 1
                            elif top1_label != box['gt_label']:
                                # Modelo confiante e discorda do GT: relabel para top1
                                self._apply_relabel(
                                    datasets,
                                    box['sub_idx'],
                                    box['data_idx'],
                                ...
                                box['gt_label'] = top1_label
                                box['score_gt'] = box['scores'][top1_label].item()
                                box['relabeled_by'] = 'margin'
                                clustering_relabel_count += 1
                                margin_count['relabeled'] += 1
                            else:
                                # Modelo confiante e top1 == GT: nada a fazer
                                margin_count['kept'] += 1
                            continue
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Relabelados por clustering: {clustering_relabel_count} "
                      f"({clustering_relabel_count/len(all_box_data)*100:.2f}%)")
                if gate_active:
                    print(f"[VCNC-KM-CM] Margin (par ruidoso): "
                          f"relabeled={margin_count['relabeled']}, "
                          f"filtered={margin_count['filtered']}, "
                          f"kept={margin_count['kept']}")
        ...
        if self.enable_spatial_refinement:
            if self.debug:
                print(f"\n[VCNC-KM-CM] ETAPA 3: Spatial Refinement (threshold={self.spatial_difficulty_threshold})")
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Boxes com alta contaminação: {spatial_stats['high_contamination']}")
                print(f"[VCNC-KM-CM] Relabelados por spatial: {spatial_relabel_count} "
        ...
        if self.enable_gmm_filter:
            if self.debug:
                print(f"\n[VCNC-KM-CM] ETAPA 4: Filtragem GMM (threshold={self.filter_gmm_threshold})")
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Filtrados (ignore_flag=1): {filter_count} "
        ...
        if self.debug:
            ...
            etapa1_filter = gate_filter_count['confidence']
            print(f"\n[VCNC-KM-CM] ===== Resumo Época {epoch} =====")
            print(f"[VCNC-KM-CM] Total de boxes: {len(all_box_data)}")
            print(f"[VCNC-KM-CM] Relabel confiança: {confidence_relabel_count} ({confidence_relabel_count/len(all_box_data)*100:.2f}%)")
            print(f"[VCNC-KM-CM] Relabel clustering (anchor + margin): {clustering_relabel_count} ({clustering_relabel_count/len(all_box_data)*100:.2f}%)")
            print(f"[VCNC-KM-CM] Relabel spatial: {spatial_relabel_count} ({spatial_relabel_count/len(all_box_data)*100:.2f}%)")
            print(f"[VCNC-KM-CM] Total relabels: {total_relabels} ({total_relabels/len(all_box_data)*100:.2f}%)")
            print(f"[VCNC-KM-CM] Gate Etapa 1 (filter): {etapa1_filter}")
            print(f"[VCNC-KM-CM] Gate Etapa 2 (margin): rel={margin_count['relabeled']}, "
                  f"filt={margin_count['filtered']}, kept={margin_count['kept']}")
            print(f"[VCNC-KM-CM] Filtrados (etapa 4): {filter_count} ({filter_count/len(all_box_data)*100:.2f}%)")
            print(f"[VCNC-KM-CM] ==========================================\n")
    ...
    def _reload_datasets(self, runner):
        ...
        try:
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Erro ao recarregar: {e}")
    ...
    def _apply_relabel(self, datasets, sub_idx, data_idx, gt_idx, new_label):
        try:
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Erro relabel: {e}")
    ...
    def _apply_ignore_flag(self, datasets, sub_idx, data_idx, gt_idx):
        try:
            ...
            if self.debug:
                print(f"[VCNC-KM-CM] Erro ignore_flag: {e}")