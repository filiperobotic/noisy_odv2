...
# --- Safe ROIAlign (for backbone/neck feature pooling) ---
try:
...
try:
    from mmcv.ops import roi_align as _mmcv_roi_align
except Exception:
    _mmcv_roi_align = None

@torch.no_grad()
def _pick_feat_level(feat_maps):
    """Pick a single feature level (highest resolution) from extract_feat output."""
    # MMDet extract_feat usually returns a tuple/list of levels [P2..P5], each is [B,C,H,W]
    if isinstance(feat_maps, (list, tuple)) and len(feat_maps) > 0:
        return feat_maps[0]
    return feat_maps

@torch.no_grad()
def _roi_pool_from_featmap(featmap_bchw: torch.Tensor, boxes_xyxy: torch.Tensor, out_size: int = 1):
    """Pool per-box visual descriptors from a single feature map using ROIAlign.

    Args:
        featmap_bchw: Tensor [B,C,Hf,Wf]
        boxes_xyxy:  Tensor [N,4] in input image pixel coordinates (x1,y1,x2,y2)
        out_size: output spatial size for ROIAlign (1 -> global pooled vector)

    Returns:
        feats: Tensor [N,C] pooled features (flattened).
    """
    if featmap_bchw is None or featmap_bchw.numel() == 0 or boxes_xyxy is None or boxes_xyxy.numel() == 0:
        device = featmap_bchw.device if featmap_bchw is not None else (boxes_xyxy.device if boxes_xyxy is not None else 'cpu')
        return torch.zeros((0, 1), device=device)

    B, C, Hf, Wf = featmap_bchw.shape
    device = featmap_bchw.device
    boxes_xyxy = boxes_xyxy.to(device)

    # Infer spatial_scale from feature/input sizes.
    # Assumption: boxes are in input-image pixels after preprocessing to (H_img, W_img).
    # spatial_scale maps input coords -> feature coords.
    # Use width scale (more stable for non-square) and average with height scale.
    # NOTE: if your pipeline changes aspect ratio, this still works because both scales are inferred.
    # Here we do not need H_img/W_img explicitly.
    # If featmap comes from FPN level-0, scale is approx 1/stride.
    # We approximate stride from feature map resolution vs current input resolution.
    # The ROIAlign ops accept a single spatial_scale, so we average the two.
    # (Works well in practice for near-isotropic resize.)
    #
    # IMPORTANT: This expects `boxes_xyxy` to be in the same resized space as `inputs`.

    # Build rois with batch indices = 0..B-1; here we pool from a *single* image at a time,
    # so callers should pass featmap already indexed to [1,C,Hf,Wf] when needed.
    if B != 1:
        # If batch > 1, caller should slice before calling. We keep a safe fallback.
        # Take the first image.
        featmap_bchw = featmap_bchw[:1]
        B = 1

    # spatial_scale inferred by assuming current input width/height correspond to feature map width/height
    # We can only do this if caller provides boxes in the current input coordinate system.
    # Estimate input W,H by the max box coords (robust to padding).
    max_x2 = float(boxes_xyxy[:, 2].max().clamp_min(1.0).item())
    max_y2 = float(boxes_xyxy[:, 3].max().clamp_min(1.0).item())
    sx = float(Wf) / max_x2
    sy = float(Hf) / max_y2
    spatial_scale = float(0.5 * (sx + sy))

    rois = torch.cat([
        torch.zeros((boxes_xyxy.size(0), 1), device=device, dtype=boxes_xyxy.dtype),
        boxes_xyxy
    ], dim=1)  # [N,5]

    output_size = (int(out_size), int(out_size))

    if _tv_roi_align is not None:
        pooled = _tv_roi_align(featmap_bchw, rois, output_size=output_size, spatial_scale=spatial_scale, aligned=True)
    elif _mmcv_roi_align is not None:
        # mmcv roi_align signature: roi_align(input, rois, output_size, spatial_scale, sampling_ratio=0, pool_mode='avg', aligned=True)
        pooled = _mmcv_roi_align(featmap_bchw, rois, output_size=output_size, spatial_scale=spatial_scale, sampling_ratio=0, pool_mode='avg', aligned=True)
    else:
        # No ROIAlign available; fallback to zeros (caller should handle)
        return torch.zeros((boxes_xyxy.size(0), C), device=device)

    # pooled: [N,C,out_size,out_size] -> [N,C]
    pooled = pooled.mean(dim=(2, 3))
    return pooled


@torch.no_grad()
def _visual_feats_from_model(runner_model, inputs: torch.Tensor, img_idx: int, boxes_xyxy: torch.Tensor, out_size: int = 1):
    """Extract per-box visual descriptors from the model's backbone/neck feature maps.

    We call `extract_feat(inputs)` once per batch outside when possible and pass the feature maps.
    This helper exists as a fallback if you don't have `feat_maps` in scope.
    """
    try:
        feat_maps = runner_model.extract_feat(inputs)
        lvl0 = _pick_feat_level(feat_maps)  # [B,C,Hf,Wf]
        feat_1 = lvl0[img_idx:img_idx+1]
        return _roi_pool_from_featmap(feat_1, boxes_xyxy, out_size=out_size)
    except Exception:
        # Fallback handled by caller
        return None

# --- Faster R-CNN native RoI feature extractor ---
@torch.no_grad()
def _roi_feats_frcnn(runner_model, feat_maps, img_idx: int, boxes_xyxy: torch.Tensor):
    """Faster R-CNN native RoI feature extraction (uses roi_head.bbox_roi_extractor).

    Returns per-box features [N, C] pooled from roi_feats (mean over spatial dims).
    Falls back to None if extractor is not available.
    """
    try:
        if runner_model is None or feat_maps is None:
            return None
        if not hasattr(runner_model, 'roi_head'):
            return None
        roi_head = runner_model.roi_head
        if not hasattr(roi_head, 'bbox_roi_extractor'):
            return None
        if boxes_xyxy is None or boxes_xyxy.numel() == 0:
            return boxes_xyxy.new_zeros((0, 1))

        device = boxes_xyxy.device
        N = boxes_xyxy.size(0)
        batch_ind = torch.full((N, 1), float(img_idx), device=device, dtype=boxes_xyxy.dtype)
        rois = torch.cat([batch_ind, boxes_xyxy], dim=1).float()  # [N,5] (batch_idx, x1,y1,x2,y2)

        roi_feats = roi_head.bbox_roi_extractor(feat_maps, rois)  # [N,C,7,7] typically
        if roi_feats.dim() == 4:
            return roi_feats.mean(dim=(2, 3))  # [N,C]
        if roi_feats.dim() == 2:
            return roi_feats
    ...
    def before_train_epoch(self, runner):
        ...
        for data_batch in dataloader:
            with torch.no_grad():
                ...
                # cache backbone/neck features once per batch (for RoI feature pooling)
                try:
                    feat_maps_batch = runner.model.extract_feat(inputs)
                except Exception:
                    feat_maps_batch = None
            ...
            for i, ds in enumerate(data_batch['data_samples']):
                ...
                # --- visual descriptors BEFORE logits (Faster R-CNN native RoI features) ---
                try:
                    gt_boxes_xyxy = gt.bboxes.tensor
                    sel_gt_xyxy = gt_boxes_xyxy[node_img_local_to_valid]
                    vis_feats_tmp = _roi_feats_frcnn(runner.model, feat_maps_batch, i, sel_gt_xyxy)
                    if vis_feats_tmp is None:
                        # fallback: old manual ROIAlign on the first FPN level
                        lvl0 = _pick_feat_level(feat_maps_batch)
                        feat_1 = lvl0[i:i+1]
                        vis_feats = _roi_pool_from_featmap(feat_1, sel_gt_xyxy, out_size=1)
                    else:
                        vis_feats = vis_feats_tmp
                except Exception as e:
                    raise RuntimeError(
                        f"[GraphRelabel] Failed to extract visual RoI features (Faster R-CNN). "
                        f"This should not silently fall back to logits. Original error: {e}"
                    )
        ...
        for data_batch in dataloader:
            ...
            for i, ds in enumerate(data_batch['data_samples']):
                ...
                try:
                    gt_boxes_xyxy = gt.bboxes.tensor
                    sel_gt_xyxy_tmp = gt_boxes_xyxy[node_img_local_to_valid]
                    vis_feats_tmp = _roi_feats_frcnn(runner.model, feat_maps_batch, i, sel_gt_xyxy_tmp)
                    if vis_feats_tmp is None:
                        lvl0 = _pick_feat_level(feat_maps_batch)
                        feat_1 = lvl0[i:i+1]
                        vis_feats = _roi_pool_from_featmap(feat_1, sel_gt_xyxy_tmp, out_size=1)
                    else:
                        vis_feats = vis_feats_tmp
                except Exception as e:
                    raise RuntimeError(
    ...
    def before_train_epoch(self, runner):
        ...
        for data_batch in dataloader:
            ...
            for i, ds in enumerate(data_batch['data_samples']):
                ...
                # trusted: detector muito confiante E concorda com o rótulo atual (GT do dataset já pode ter sido atualizado)
                pmax_img, pred_cls_img = pr.max(dim=1)
                trusted = (pmax_img >= self.trust_thr) & (pred_cls_img == node_labels_t)

                if trusted.any():
                    loss_cong = F.cross_entropy(logits_cong[trusted], node_labels_t[trusted])
                    if batch_loss is None:
                        batch_loss = loss_cong
                    else:
                        batch_loss = batch_loss + loss_cong
