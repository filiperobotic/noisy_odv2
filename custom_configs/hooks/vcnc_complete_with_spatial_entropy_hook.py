"""
Estratégia D: Decisão Baseada em Entropia do Softmax

Ideia:
- Ruído simétrico: modelo tem alta confiança em uma classe errada (entropia baixa)
- Ruído assimétrico: modelo fica indeciso entre classes similares (entropia alta no top-K)

Lógica:
- Calcula entropia do softmax (ou entropia concentrada no top-K)
- Entropia baixa + score_gt baixo → simétrico → RELABELA
- Entropia alta + score_gt baixo → assimétrico → FILTRA
"""
...
def compute_topk_entropy(scores, k=3):
    """
    Calcula entropia concentrada no top-K classes.
    
    Alta entropia no top-K → modelo indeciso entre classes similares (assimétrico)
    Baixa entropia no top-K → modelo confiante em uma classe (simétrico ou correto)
    """
    topk_scores, _ = torch.topk(scores, min(k, len(scores)))
    # Normaliza para somar 1
    topk_probs = topk_scores / (topk_scores.sum() + 1e-8)
    # Entropia
    entropy = -torch.sum(topk_probs * torch.log(topk_probs + 1e-8))
    # Normaliza pela entropia máxima (log(k))
...
class VCNCEntropyBasedHook(Hook):
    """
    VCNC Estratégia D: Decisão baseada em Entropia
    
    Lógica:
    - Entropia baixa (< threshold) + suspeito → RELABELA (simétrico)
    - Entropia alta (>= threshold) + suspeito → FILTRA (assimétrico)
    """
    ...
    def __init__(self,
                 ...
                 # Threshold de entropia
                 entropy_threshold: float = 0.5,  # Acima disso = assimétrico
                 top_k_entropy: int = 3,  # Calcular entropia no top-K
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[EntropyBased] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[EntropyBased] ========== Época {epoch} ==========")
        ...
        for batch_idx, data_batch in enumerate(dataloader):
            ...
            for i, data_sample in enumerate(data_batch['data_samples']):
                ...
                for gt_idx in range(assign_result.num_gts):
                    ...
                    # Calcular entropia no top-K
                    topk_entropy = compute_topk_entropy(best_scores, k=self.top_k_entropy)
        ...
        if self.debug:
            print(f"[EntropyBased] Total boxes: {len(all_box_data)}")
            entropies = [b['topk_entropy'] for b in all_box_data]
            print(f"[EntropyBased] Entropia média: {np.mean(entropies):.3f}, std: {np.std(entropies):.3f}")
        ...
        if self.debug:
            print(f"[EntropyBased] Fase: {criteria['phase']}")
        ...
        stats = {
            'low_entropy_relabel': 0,
            'high_entropy_filter': 0,
        }
        ...
        for cluster_id, cluster_boxes in clusters.items():
            ...
            for box in cluster_boxes:
                ...
                if similarity > c_similarity:
                    # ============================================================
                    # DECISÃO BASEADA EM ENTROPIA
                    # ============================================================
                    if box['topk_entropy'] < self.entropy_threshold:
                        # Entropia baixa → modelo confiante → simétrico → RELABELA
                        self._apply_relabel(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'], dominant_label)
                        box['relabeled'] = True
                        relabel_count += 1
                        stats['low_entropy_relabel'] += 1
                    else:
                        # Entropia alta → modelo indeciso → assimétrico → FILTRA
                        self._apply_ignore_flag(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'])
                        box['filtered'] = True
                        filter_count += 1
                        stats['high_entropy_filter'] += 1
        
        if self.debug:
            print(f"[EntropyBased] Total âncoras: {total_anchors} ({total_anchors/len(all_box_data)*100:.2f}%)")
            print(f"[EntropyBased] Total suspeitos: {total_suspects}")
            print(f"[EntropyBased] Decisões (threshold={self.entropy_threshold}):")
            print(f"  - Entropia baixa → Relabel: {stats['low_entropy_relabel']}")
            print(f"  - Entropia alta → Filtra: {stats['high_entropy_filter']}")
        ...
        if self.enable_spatial_refinement:
            ...
            if self.debug:
                print(f"[EntropyBased] Relabelados por spatial: {spatial_relabel_count}")
        
        if self.debug:
            print(f"[EntropyBased] ==========================================\n")
