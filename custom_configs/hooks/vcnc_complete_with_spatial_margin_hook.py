"""
Estratégia E: Decisão Baseada em Margem entre Top-1 e Top-2

Ideia:
- Margem grande: modelo tem certeza → pode ser simétrico ou correto
- Margem pequena: modelo indeciso entre 2 classes → pode ser assimétrico

Lógica:
- Margem > threshold + suspeito → RELABELA (simétrico)
- Margem <= threshold + suspeito → FILTRA (assimétrico)
"""
...
class VCNCMarginBasedHook(Hook):
    """
    VCNC Estratégia E: Decisão baseada em Margem Top1-Top2
    
    Lógica:
    - Margem grande (> threshold) + suspeito → RELABELA (simétrico)
    - Margem pequena (<= threshold) + suspeito → FILTRA (assimétrico)
    """
    ...
    def __init__(self,
                 ...
                 # Threshold de margem
                 margin_threshold: float = 0.3,  # Abaixo disso = assimétrico
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[MarginBased] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[MarginBased] ========== Época {epoch} ==========")
        ...
        for batch_idx, data_batch in enumerate(dataloader):
            ...
            for i, data_sample in enumerate(data_batch['data_samples']):
                ...
                for gt_idx in range(assign_result.num_gts):
                    ...
                    # Calcular margem entre Top-1 e Top-2
                    top2_scores, top2_labels = best_scores.topk(2)
        ...
        if self.debug:
            print(f"[MarginBased] Total boxes: {len(all_box_data)}")
            margins = [b['margin'] for b in all_box_data]
            print(f"[MarginBased] Margem média: {np.mean(margins):.3f}, std: {np.std(margins):.3f}")
        ...
        if self.debug:
            print(f"[MarginBased] Fase: {criteria['phase']}")
        ...
        if self.debug:
            print(f"[MarginBased] Total âncoras: {total_anchors} ({total_anchors/len(all_box_data)*100:.2f}%)")
            print(f"[MarginBased] Total suspeitos: {total_suspects}")
            print(f"[MarginBased] Decisões (threshold={self.margin_threshold}):")
            print(f"  - Margem grande → Relabel: {stats['large_margin_relabel']}")
        ...
        if self.enable_spatial_refinement:
            ...
            if self.debug:
                print(f"[MarginBased] Relabelados por spatial: {spatial_relabel_count}")
