"""
Estratégia F: Decisão Baseada em GT no Top-K

Ideia:
- Se GT está no top-K com score razoável → confusão entre classes similares (assimétrico)
- Se GT não está no top-K ou tem score muito baixo → ruído simétrico

Lógica:
- GT no top-2 com score > min_score → FILTRA (assimétrico)
- GT fora do top-3 ou score muito baixo → RELABELA (simétrico)
"""
...
class VCNCGTInTopKHook(Hook):
    """
    VCNC Estratégia F: Decisão baseada em GT no Top-K
    
    Lógica:
    - GT no top-K com score >= min_score → FILTRA (assimétrico)
    - GT fora do top-K ou score muito baixo → RELABELA (simétrico)
    """
    ...
    def __init__(self,
                 ...
                 # Parâmetros do GT no Top-K
                 top_k: int = 2,  # Verificar se GT está no top-K
                 min_gt_score_for_asymmetric: float = 0.1,  # Se GT no top-K E score >= isso → assimétrico
        ...
        self.top_k = top_k
        self.min_gt_score_for_asymmetric = min_gt_score_for_asymmetric
    ...
    def _check_gt_in_topk(self, scores, gt_label, k):
        """Verifica se GT está no top-K e retorna sua posição e score."""
        topk_scores, topk_labels = scores.topk(k)
        
        gt_in_topk = gt_label in topk_labels.tolist()
        gt_rank = None
        
        if gt_in_topk:
            for i, label in enumerate(topk_labels.tolist()):
                if label == gt_label:
                    gt_rank = i + 1  # 1-indexed
                    break
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[GTInTopK] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[GTInTopK] ========== Época {epoch} ==========")
        ...
        for batch_idx, data_batch in enumerate(dataloader):
            ...
            for i, data_sample in enumerate(data_batch['data_samples']):
                ...
                for gt_idx in range(assign_result.num_gts):
                    ...
                    # Verificar se GT está no top-K
                    gt_in_topk, gt_rank = self._check_gt_in_topk(best_scores, gt_label, self.top_k)
        ...
        if self.debug:
            print(f"[GTInTopK] Total boxes: {len(all_box_data)}")
            gt_in_topk_count = sum(1 for b in all_box_data if b['gt_in_topk'])
            print(f"[GTInTopK] GT no top-{self.top_k}: {gt_in_topk_count} ({gt_in_topk_count/len(all_box_data)*100:.1f}%)")
        ...
        if self.debug:
            print(f"[GTInTopK] Fase: {criteria['phase']}")
        ...
        for cluster_id, cluster_boxes in clusters.items():
            ...
            for box in cluster_boxes:
                ...
                if similarity > c_similarity:
                    # ============================================================
                    # DECISÃO BASEADA EM GT NO TOP-K
                    # ============================================================
                    if box['gt_in_topk'] and box['score_gt'] >= self.min_gt_score_for_asymmetric:
                        # GT no top-K com score razoável → assimétrico → FILTRA
                        self._apply_ignore_flag(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'])
                        box['filtered'] = True
                        filter_count += 1
                        stats['gt_in_topk_filter'] += 1
                    else:
                        # GT fora do top-K ou score muito baixo → simétrico → RELABELA
                        self._apply_relabel(datasets, box['sub_idx'], box['data_idx'], box['gt_idx'], dominant_label)
                        box['relabeled'] = True
                        relabel_count += 1
                        stats['gt_not_in_topk_relabel'] += 1
        
        if self.debug:
            print(f"[GTInTopK] Total âncoras: {total_anchors} ({total_anchors/len(all_box_data)*100:.2f}%)")
            print(f"[GTInTopK] Total suspeitos: {total_suspects}")
            print(f"[GTInTopK] Decisões (top_k={self.top_k}, min_score={self.min_gt_score_for_asymmetric}):")
            print(f"  - GT no top-K → Filtra: {stats['gt_in_topk_filter']}")
            print(f"  - GT fora top-K → Relabel: {stats['gt_not_in_topk_relabel']}")
        ...
        if self.enable_spatial_refinement:
            ...
            if self.debug:
                print(f"[GTInTopK] Relabelados por spatial: {spatial_relabel_count}")
