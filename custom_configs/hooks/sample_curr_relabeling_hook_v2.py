...
@HOOKS.register_module()
class MyHookReverseCurrIntoFilterPredGT_Class_Relabel(Hook):

    def __init__(self, reload_dataset=False, relabel_conf=0.95, double_thr = 2, filter_conf=0.8, filter_warmup=0, iou_assigner=0.5, low_quality=False, filter_thr=0.7, numGMM=2, filter_type = 'pred', group = False, selcand='max', overlap_filter_start=6, overlap_filter_end=10, overlap_iou_thr=0.0):
        """
        Hook personalizado para relabeling de amostras.
        ...
        # In the first epochs, ignore GT instances that overlap any other GT in the same image.
        # This is useful for very noisy data where co-occurrences (e.g., person on horse/sofa) can confuse early training.
        self.overlap_filter_start = overlap_filter_start  
        self.overlap_filter_end = overlap_filter_end      
        self.overlap_iou_thr = overlap_iou_thr
        ...
        # if (runner.epoch + 1) >=5 :  
            ...
            for batch_idx, data_batch in enumerate(dataloader):
                ...
                # for i, data_sample in enumerate(data['data_samples']):   #after pre_process
                    ...
                    to_ignore_gt = set()
                    # if ((runner.epoch + 1) <= self.overlap_filter_epochs) and  ((runner.epoch + 1) >= self.filter_warmup) and (float(self.overlap_iou_thr) > 0.0):
                    if ((runner.epoch + 1) >= self.overlap_filter_start and (runner.epoch + 1) <= self.overlap_filter_end and (runner.epoch + 1) >= self.filter_warmup and  (float(self.overlap_iou_thr) > 0.0)):
                        sub_dataset_idx, dataset_data_idx = dataset_img_map[img_path]
                        sub_dataset = datasets[sub_dataset_idx]
            ...
            # if (runner.epoch + 1) >= 0:
                ...
                for c in range(num_classes):
                    ...
                    total_gmm_filtered = len(all_classes_low_confidence_scores_global_idx)
                    print(f"[GMM-FILTER] ep={runner.epoch + 1}: "
                        f"filtered {total_gmm_filtered}/{global_index_counter} GTs "
                        f"({100*total_gmm_filtered/global_index_counter:.1f}%)")
...
@HOOKS.register_module()
class MyHookRFirstCurrIntoFilterPredGT_Class_Relabel(Hook):
