"""
VCNC com Detecção de Pares Contaminados

Problema: No ruído assimétrico, âncoras erradas (gato rotulado como cachorro)
passam nos critérios porque o modelo aprende a associação errada.

Insight: Mesmo que o modelo prediga "cachorro" (top-1), o top-2 frequentemente
será "gato" (a classe verdadeira). Isso cria um padrão detectável:

  Âncoras de "cachorro" contaminadas por gatos:
    - Top-1: cachorro
    - Top-2: gato (concentrado!)
    
  Âncoras de "cachorro" limpas:
    - Top-1: cachorro  
    - Top-2: varia (cão, cavalo, etc.)

Solução:
1. Para cada classe, analisar distribuição de top-2 nas âncoras
2. Detectar pares (A, B) onde B aparece muito como top-2 de âncoras de A
3. Para esses pares, ser mais restritivo (filtrar ao invés de relabeling)
"""
...
class VCNCContaminationDetectionHook(Hook):
    """
    VCNC com Detecção de Pares Contaminados.
    
    Detecta pares de classes (A, B) onde há suspeita de ruído assimétrico A → B
    analisando a concentração de top-2 nas âncoras.
    
    Para pares contaminados, usa FILTRAGEM ao invés de RELABELING.
    """
    ...
    def __init__(self,
                 ...
                 contamination_threshold: float = 0.3,  # Se >30% das âncoras de A têm top2=B
                 min_anchors_for_detection: int = 20,   # Mínimo de âncoras para analisar
        ...
        self.min_anchors_for_detection = min_anchors_for_detection
        ...
        # Pares contaminados detectados (atualizado por época)
        self.contaminated_pairs = set()  # Conjunto de tuplas (source, target)
    ...
    def _detect_contaminated_pairs(self, anchors):
        """
        Detecta pares de classes com suspeita de ruído assimétrico.
        
        Analisa a distribuição de top-2 nas âncoras de cada classe.
        Se uma classe tem top-2 muito concentrado em outra classe específica,
        isso indica possível contaminação.
        
        Returns:
            contaminated_pairs: set de tuplas (source_class, target_class)
                onde source_class → target_class é o ruído suspeito
        """
        if not self.enable_contamination_detection:
            return set()
        
        # Agrupar âncoras por GT
        anchors_by_class = defaultdict(list)
        for anchor in anchors:
            anchors_by_class[anchor['gt_label']].append(anchor)
        
        contaminated_pairs = set()
        contamination_stats = {}
        
        for gt_class, class_anchors in anchors_by_class.items():
            if len(class_anchors) < self.min_anchors_for_detection:
                continue
            
            # Contar distribuição de top-2
            top2_counts = Counter()
            for anchor in class_anchors:
                top2_label = anchor['top2_label']
                if top2_label != gt_class:  # Ignorar se top2 == GT
                    top2_counts[top2_label] += 1
            
            if len(top2_counts) == 0:
                continue
            
            # Verificar se há concentração alta em alguma classe
            total_with_different_top2 = sum(top2_counts.values())
            
            for top2_class, count in top2_counts.most_common():
                concentration = count / len(class_anchors)
                
                if concentration >= self.contamination_threshold:
                    # Suspeita: ruído top2_class → gt_class
                    contaminated_pairs.add((top2_class, gt_class))
                    
                    if self.debug:
                        print(f"[VCNC-Contam] DETECTADO: {top2_class} → {gt_class} "
                              f"({count}/{len(class_anchors)} = {concentration*100:.1f}%)")
                    
                    contamination_stats[(top2_class, gt_class)] = {
                        'count': count,
                        'total_anchors': len(class_anchors),
                        'concentration': concentration
                    }
        
        return contaminated_pairs, contamination_stats
    
    def _is_contaminated_relabel(self, suspect_gt, dominant_label):
        """
        Verifica se relabeling suspect_gt → dominant_label é entre pares contaminados.
        
        Se for par contaminado, devemos FILTRAR ao invés de RELABELING.
        """
        # Checamos ambas as direções
        return (suspect_gt, dominant_label) in self.contaminated_pairs or \
               (dominant_label, suspect_gt) in self.contaminated_pairs
    ...
    def before_train_epoch(self, runner):
        ...
        if epoch <= self.warmup_epochs:
            if self.debug:
                print(f"[VCNC-Contam] Época {epoch}: Warmup, pulando.")
            return
        
        if self.debug:
            print(f"\n[VCNC-Contam] ========== Época {epoch} ==========")
        ...
        if len(all_box_data) == 0:
            print("[VCNC-Contam] Nenhum box coletado!")
            return
        
        if self.debug:
            print(f"[VCNC-Contam] Coletados {len(all_box_data)} boxes")
        ...
        if self.enable_confidence_relabel:
            ...
            if self.debug:
                print(f"[VCNC-Contam] Relabelados por confiança: {confidence_relabel_count}")
        
        # ============================================================
        # ETAPA 2: CLUSTERING E DETECÇÃO DE CONTAMINAÇÃO
        # ============================================================
        clustering_relabel_count = 0
        contaminated_filter_count = 0
        ...
        if self.enable_clustering_relabel:
            ...
            # Primeiro passo: identificar todas as âncoras
            all_anchors = []
            for cluster_id, cluster_boxes in clusters.items():
                for box in cluster_boxes:
                    ...
                    if is_clean and model_agrees and high_confidence:
                        box['is_anchor'] = True
                        all_anchors.append(box)
                    else:
                        box['is_anchor'] = False
            
            if self.debug:
                print(f"[VCNC-Contam] Total âncoras: {len(all_anchors)} ({len(all_anchors)/len(all_box_data)*100:.1f}%)")
            
            # Detectar pares contaminados
            self.contaminated_pairs, contamination_stats = self._detect_contaminated_pairs(all_anchors)
            
            if self.debug:
                print(f"[VCNC-Contam] Pares contaminados detectados: {len(self.contaminated_pairs)}")
                for pair in self.contaminated_pairs:
                    print(f"[VCNC-Contam]   {pair[0]} → {pair[1]}")
            
            # Diagnóstico
            all_suspects_treated = []
            all_suspects_filtered_contaminated = []
            all_suspects_not_similar = []
            all_noisy_not_treated = []
            ...
            for cluster_id, cluster_boxes in clusters.items():
                ...
                anchors = [b for b in cluster_boxes if b.get('is_anchor', False)]
                
                if len(anchors) == 0:
                    for box in cluster_boxes:
                        if box['p_noise'] >= c_suspect_gmm:
                            all_noisy_not_treated.append(box)
                    continue
                ...
                if consensus_ratio < c_consensus:
                    for box in cluster_boxes:
                        if box['p_noise'] >= c_suspect_gmm and box['gt_label'] != dominant_label:
                            all_noisy_not_treated.append(box)
                    continue
                ...
                for box in cluster_boxes:
                    ...
                    box['similarity_to_anchors'] = similarity
                    
                    if similarity > c_similarity:
                        # Verificar se é par contaminado
                        if self._is_contaminated_relabel(box['gt_label'], dominant_label):
                            # PAR CONTAMINADO → FILTRAR ao invés de relabeling
                            ...
                            box['filtered'] = True
                            box['filtered_reason'] = 'contaminated_pair'
                            contaminated_filter_count += 1
                            all_suspects_filtered_contaminated.append(box)
                        else:
                            # Par não contaminado → RELABELING normal
                            self._apply_relabel(
                                datasets,
                                box['sub_idx'],
                                box['data_idx'],
                            ...
                            all_suspects_treated.append(box)
                    else:
                        all_suspects_not_similar.append(box)
            
            if self.debug:
                print(f"[VCNC-Contam] Relabelados por clustering: {clustering_relabel_count}")
                print(f"[VCNC-Contam] FILTRADOS por par contaminado: {contaminated_filter_count}")
                print(f"[VCNC-Contam] Suspeitos não similares: {len(all_suspects_not_similar)}")
                print(f"[VCNC-Contam] Ruidosos sem consenso: {len(all_noisy_not_treated)}")
        ...
        if self.enable_spatial_refinement:
            ...
            if self.debug:
                print(f"[VCNC-Contam] Relabelados por spatial: {spatial_relabel_count}")
        ...
        if self.enable_gmm_filter:
            ...
            if self.debug:
                print(f"[VCNC-Contam] Filtrados por GMM: {gmm_filter_count}")
        ...
        if self.enable_diagnostic:
            stats = {
                ...
                'contaminated_pairs': list(self.contaminated_pairs),
                'contamination_stats': {f"{k[0]}->{k[1]}": v for k, v in contamination_stats.items()},
                'confidence_relabel': confidence_relabel_count,
                'clustering_relabel': clustering_relabel_count,
                'contaminated_filter': contaminated_filter_count,
                'spatial_relabel': spatial_relabel_count,
                'gmm_filter': gmm_filter_count,
                'suspects_not_similar': len(all_suspects_not_similar),
                'noisy_not_treated': len(all_noisy_not_treated),
            }
            ...
            print(f"\n[VCNC-Contam] ========== DIAGNÓSTICO ==========")
            print(f"[VCNC-Contam] Pares contaminados: {list(self.contaminated_pairs)}")
            print(f"[VCNC-Contam] Filtrados por contaminação: {contaminated_filter_count}")
            print(f"[VCNC-Contam] =====================================\n")
    ...
    def _apply_relabel(self, datasets, sub_idx, data_idx, gt_idx, new_label):
        try:
            ...
            if self.debug:
                print(f"[VCNC-Contam] Erro relabel: {e}")
    ...
    def _apply_ignore_flag(self, datasets, sub_idx, data_idx, gt_idx):
        try:
            ...
            if self.debug:
                print(f"[VCNC-Contam] Erro ignore_flag: {e}")