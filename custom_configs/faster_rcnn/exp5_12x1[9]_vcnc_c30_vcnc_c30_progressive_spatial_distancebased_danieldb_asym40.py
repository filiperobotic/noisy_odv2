...
custom_imports = dict(
    imports=['custom_configs.hooks.vcnc_complete_with_spatial_distancebased_hook',
...
custom_hooks = [
    dict(
    type='VCNCDistanceBasedHook',
    warmup_epochs=1,
    num_classes=20,
    
    # Thresholds de distância (similaridade cosseno)
    close_threshold=0.7,    # > 0.7 = perto
    far_threshold=0.3,      # < 0.3 = longe
    
    # Threshold de score_gt
    symmetric_score_threshold=0.05,  # < 0.05 → relabela (simétrico)
