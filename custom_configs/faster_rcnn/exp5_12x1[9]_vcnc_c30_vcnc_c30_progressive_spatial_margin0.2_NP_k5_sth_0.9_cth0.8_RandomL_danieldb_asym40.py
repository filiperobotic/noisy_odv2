...
custom_imports = dict(
    imports=['custom_configs.hooks.vcnc_complete_with_spatial_noisePropag_hook',
...
custom_hooks = [
    dict(
    type='VCNCNoisePropagationHook',
        warmup_epochs=1,
        num_classes=20,
        margin_threshold=0.2,
        # Propagação
        enable_propagation=True,
        propagation_k=5,  # Busca 5 vizinhos
        propagation_similarity_threshold=0.9,  # Similaridade alta para propagar
        propagation_confidence_threshold=0.8,  # p_noise >= 0.8 para ser fonte
        # Ação
        use_random_label=True,  # Random label ao invés de filtrar
        exclude_gt_and_pred=True,
