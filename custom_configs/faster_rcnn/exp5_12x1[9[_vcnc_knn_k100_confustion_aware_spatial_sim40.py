...
custom_imports = dict(
    imports=['custom_configs.hooks.vcnc_knn_confusion_aware_spatial_hook',
...
custom_hooks = [
    ...
    dict(
        type='VCNCKNNConfusionAwareHook',
 
        # === Configuração geral (igual ao seu vcnc_knn_k100_spatial) ===
        warmup_epochs=1,
        num_classes=20,
        ...
        # === Etapa 2 — KNN ===
        enable_knn_relabel=True,
        use_softmax_as_embedding=True,
        knn_k=100,
        knn_min_anchors=5,
        knn_consensus_threshold=0.6,
        knn_distance_weighted=True,
 
        # Critérios progressivos (mesmos defaults do KNN base)
        ...
        enable_confusion_gate=True,           # ablação: False → idêntico ao KNN base
        confusion_gate_min_samples=50,        # reusa o mesmo mínimo de outros hooks
        confusion_gate_mad_factor=3.0,        # 3-sigma (convenção)
        confusion_gate_ratio_factor=10.0,     # 10x mediana (uma ordem de grandeza)
        confusion_gate_aggressive_only=False, # gate ativo desde o fim do warmup
 
        # === Etapa 3 — Spatial ===
        enable_spatial_refinement=True,
        spatial_difficulty_threshold=0.5,
 
        # === Etapas 4 e 5 — opcionais, off por padrão como no KNN base ===
