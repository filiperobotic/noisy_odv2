...
custom_imports = dict(
    imports=['custom_configs.hooks.vcnc_complete_with_spatial_lowscore_only_hook',
...
custom_hooks = [
    dict(
    type='VCNCRelabelLowScoreOnlyHook',
    warmup_epochs=1,
    num_classes=20,
    
    # Thresholds de score_gt para decisão
    relabel_max_score_gt=0.05,   # score_gt < 0.05 → relabela
    filter_max_score_gt=0.30,    # 0.05 < score_gt < 0.30 → filtra
