...
custom_imports = dict(
    imports=['custom_configs.hooks.visual_clustering_noise_correction_progressive_hook',
...
custom_hooks = [
    dict(
    type='VisualClusteringNoiseCorrectionProgressiveHook',
    ...
    n_clusters=150,               # Número de clusters (recomendado: num_classes * 5)
    # Épocas com critérios conservadores (2, 3, 4)
    progressive_epochs=4,
    use_softmax_as_embedding=True,  # Usar softmax como embedding (True) ou logits (False)
    
    # Critérios CONSERVADORES para épocas 2-4
    # (mais rígidos = menos relabels)
    early_anchor_gmm_threshold=0.15,      # p_noise < 0.15 (muito limpo)
    early_anchor_pred_agreement=0.85,     # score[gt] > 0.85 (modelo muito confiante)
    early_anchor_confidence=0.9,          # max(scores) > 0.9
    early_suspect_gmm_threshold=0.8,      # p_noise > 0.8 (muito suspeito)
    early_similarity_threshold=0.7,       # similaridade > 0.7
    early_cluster_consensus=0.85,         # 85% das âncoras concordam

    # Critérios AGRESSIVOS para épocas 5+
    # (mais relaxados = mais relabels)
